...
> **Free, Gemini, and one of three.** This is the **free** door (Gemini CLI, no billed key) — *not* Anthropic's built-in `/deep-research` (which fans out Claude's own WebSearch). The **paid** Gemini door is a Gemini API skill (billed key, sharper/deeper), if you have one installed.
...
- **If it still fails** (all free models capped): use a paid Gemini API skill (billed key), if you have one, or fall back to Claude's own `/deep-research`. Don't keep retrying a quota-blocked model — that's the trap that wastes time.
