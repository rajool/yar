...
The fallback is only for a *missing* PyYAML — frontmatter that PyYAML rejects is
a hard error, never quietly re-parsed by the lenient parser, because the runtime
loader drops every field of a skill whose YAML does not parse.
"""
import os
import re
import sys
...
class FrontmatterError(Exception):
    """The frontmatter is not valid YAML — the runtime would drop every field."""


def parse_frontmatter(block):
    """With PyYAML when it is installed; a simple key: value parser when it is not.

    A PyYAML *parse* error is raised, not swallowed: the lenient fallback would
    happily read frontmatter that the real loader rejects, which is how a skill
    ships with silently empty metadata.
    """
    try:
...
def validate_skill(skill_dir):
    ...
    try:
        ...
        errors.append(
            "The YAML frontmatter does not parse, so at runtime this skill loads with "
            "no metadata at all (every field is silently dropped): %s. A common cause is "
            "an unquoted value containing \": \" — wrap the value in single quotes." % exc
        )
        return errors, warnings
