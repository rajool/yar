...
#   - home-relative paths into a specific project (~/Projects/<repo>); a bare root such
#     as ~/Projects is fine
#   - any term in an optional, gitignored .claude/hooks/context-denylist.local.txt
#   - in hook mode, also the text of Bash commands that write prose into git or GitHub
#     (git commit -m, git tag -m, gh pr|issue|release create|edit|comment), so a private
#     term cannot ride into a commit message or a pull-request body
#
...
PLACEHOLDER_EMAIL_LOCAL = {
    ...
    "noreply", "no-reply", "no_reply",  # bot / privacy senders, not a person
}
PLACEHOLDER_EMAIL_DOMAINS = (
    "example.com", "example.org", "example.net", "domain.com", "email.com",
    "test.com", "company.com", "yourcompany.com", "acme.com",
    "noreply.github.com",  # GitHub's privacy address (<user>@users.noreply.github.com)
)
PLACEHOLDER_NAMES = {
    "you", "user", "username", "name", "me", "yourname", "your-name", "youruser",
    "someone", "home", "<user>", "<you>", "<name>",
}
# Repo-name placeholders after a projects root (~/Projects/<repo>, ~/Projects/your-project).
PLACEHOLDER_REPOS = PLACEHOLDER_NAMES | {
    "repo", "project", "my-repo", "my-project", "your-repo", "your-project",
    "example", "acme", "app", "<repo>", "<project>", "path", "to",
}
...
# A home-relative path INTO a specific project. The root alone (~/Projects) is a
# generic default; the segment after it is a real machine's layout.
PROJECT_PATH_RE = re.compile(
    r"~/(?:[Pp]rojects|[Cc]ode|repos|src|dev|[Dd]eveloper|[Cc]lients|[Ww]ork)/"
    r"([^\s/`'\")\]]+)")
# Bash commands whose arguments are prose that lands in git or on GitHub.
PROSE_COMMAND_RE = re.compile(
    r"(?:^|[\s;&|(`])(?:git\s+(?:commit|tag)\b"
    r"|gh\s+(?:pr|issue|release)\s+(?:create|edit|comment|review)\b)")
...
def findings_in_line(line):
    ...
    for m in PROJECT_PATH_RE.finditer(line):
        seg = m.group(1)
        if seg.startswith(("<", "$", "{")) or seg.lower() in PLACEHOLDER_REPOS:
            continue
        out.append(("project-path", m.group(0)))
...
HELP = (
    ...
    "emails, home or project paths, secrets, or references to private/internal systems.\n"
    "\n"
    "How to continue:\n"
    "  - Real context (an email, a path, a secret, a private name)? Remove or\n"
    "    generalize it (use a placeholder: /Users/you/..., /path/to/repo, you@example.com).\n"
...
def hook_mode():
    ...
    # Bash: only commands that write prose into git/GitHub are read (commit and tag
    # messages, PR/issue/release text); everything else is none of this guard's business.
    cmd = ti.get("command")
    if isinstance(cmd, str) and cmd and PROSE_COMMAND_RE.search(cmd):
        blobs.append(("command", cmd))
