...
| [`repo-sweep`](skills/repo-sweep/SKILL.md) | `/yar:repo-sweep` | End-of-day sweep across **every** local repo, worktree, and installed plugin at once: push/pull to reconcile each repo with its remote (never discarding work), prune stale state — local branches whose content already landed (squash-merge aware, merged pull requests as proof), merged/contained worktrees (never one a live process is standing in), **orphaned worktree dirs** left by repo renames, parked primaries, zero-unique remote branches — and update every installed plugin to its latest published version, flagging manifest/install version drift. Read-only scan → classified plan (safe / needs-you / skipped) → confirmation. Builds on [`git-workflow`](skills/git-workflow/SKILL.md); triggers on "good night" / "sweep the repos". |
