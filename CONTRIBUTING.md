...
- home-relative paths into a specific project (write `/path/to/repo`, not `~/Projects/<your-repo>`)
...
- **Commit messages and pull-request text** — the same hook reads the text of
  `git commit -m` and `gh pr create|edit` commands, and the `no-context` check scans the
  pull request's title and body. For commits made outside Claude, a `commit-msg` git hook
  can run `python3 .claude/hooks/context-guard.py --scan .git/COMMIT_EDITMSG`.
